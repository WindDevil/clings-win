...
184 个练习里只有 12 个需要 Windows 变体，其余原样可用。因此孪生工程采用
「同步 + 覆盖表」结构，而不是维护两套练习。
...
| 阶段 | 结果 |
| --- | --- |
| 未修改的上游代码编译（上游自带的 185 道） | 181 / 185 |
| 未修改的上游代码运行 | 180 / 181（补齐 `libwinpthread-1.dll` 后） |
| 本仓库应用 Windows 覆盖后编译 | 184 / 184 |
| 本仓库应用 Windows 覆盖后运行 | 184 / 184 |
| 初始练习仍然失败（`selftest`） | 184 / 184 |
